    ...
    return (
        <div className="border rounded-xl p-6 font-semibold">
            <div className="grid grid-cols-[auto_2fr_1fr_auto] w-full pb-2 border-b-2 mb-2 text-zinc-500">
                ...
                <p>Product</p>
                <p>Price</p>
                <p className="w-14 text-center">Opt</p>
            </div>
                ...
                if (products[0]) {
                    ...
                    return (
                        <div className="grid grid-cols-[auto_2fr_1fr_auto] items-center w-full py-3 border-b font-semibold gap-x-2 last:border-b-0" key={index}>
                            ...
                            <p>{product.name}</p>
                            <p className="tracking-tighter font-mono"><span className="text-zinc-700">#</span>{product.price}</p>
                            <Options productId={product.id} setProducts={setProducts} />
                        </div>
                    )
                }
...
const Options = ({ productId, setProducts }: { productId: number, setProducts: (products: SavedProductDetail[]) => void }) => {
    const handleDelete = () => {
